    ...
    "AND": [
        {
            "AND": [
                {
                    "field": "atom_type",
                    "multi_values": ["phone_number"],
                    "type": "filter",
                },
                {"field": "risk", "range": {"gt": 90}, "type": "filter"},
            ]
        }
    ]
}
...
def mock_api_resp():
    responses.add(
        ...
        + TestData.TEST_CONFIG["endpoints"]["advanced-search"],
        status=200,
        json={
            "count": 1,
...
def test_advanced_search_from_query_hash_bad_ordering(datalake: Datalake):
    with pytest.raises(ValueError) as exec_error:
        datalake.AdvancedSearch.advanced_search_from_query_hash(
            query_hash, limit=0, ordering=["badbad"]
        )
    assert (
        str(exec_error.value)
        == "ordering needs to be a list of at least one of the following str : first_seen, "
...
def test_advanced_search_from_query_hash_ok_ordering(datalake: Datalake):
    mock_api_resp()
    resp = datalake.AdvancedSearch.advanced_search_from_query_hash(
        query_hash, limit=0, ordering=["first_seen"]
    )
    assert resp["query_hash"] == query_hash
