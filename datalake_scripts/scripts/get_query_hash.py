...
def main(override_args=None):
    ...
    dtl.logger.debug(f"START: get_query_hash.py")

    # Load api_endpoints and tokens
    with open(args.query_body_path, "r") as query_body_file:
        query_body = json.load(query_body_file)
    dtl.logger.debug(f"Retrieving query hash for query body: {query_body}")
    ...
    if not resp or "query_hash" not in resp:
        dtl.logger.error("Couldn't retrieve a query hash, is the query body valid ?")
        exit(1)
    query_hash = resp["query_hash"]
    if args.output:
        with open(args.output, "w") as output:
            output.write(query_hash)
        dtl.logger.info(f"Query hash saved in {args.output}")
    else:
        dtl.logger.info(f"Query hash associated: {query_hash}")
