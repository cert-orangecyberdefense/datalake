...
def load_csv(file_name: str, delimiter: str = ",", column: int = 0) -> List[str]:
    ...
    try:
        ...
        raise ValueError(
            f"Csv passed does not have enough columns on line {i} or the delimiter is incorrect"
        )
    return ret
...
def parse_threat_types(threat_types: list) -> list:
    ...
    for i in range(0, len(threat_types), 2):
        ...
        try:
            ...
            raise ValueError(
                f"Unknow threat_types: {threat_types[i]} {score},"
                f" please use only value in {[e.value for e in ThreatType]}."
            )
        if score < 0 or score > 100:
            raise ValueError(
                f"Wrong score: {threat_type} {score}, "
