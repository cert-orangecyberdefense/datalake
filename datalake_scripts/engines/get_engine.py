    ...
    def get_json(self, list_threats: list):
        ...
        for index, threat in enumerate(list_threats):
            ...
            if not response_dict.get("hashkey"):
                ...
                logger.info(
                    f"{str(index).ljust(5)}:{threat.ljust(terminal_size - 11)}\x1b[0;30;41mERROR\x1b[0m"
                )
            else:
                logger.info(
                    f"{str(index).ljust(5)}:{threat.ljust(terminal_size - 10)}\x1b[0;30;42m OK \x1b[0m"
                )
                response_dict = self.datalake_requests(
                    f"{request_url}/comments", "get", self._get_headers(), None
                )
...
class Threats(GetEngine):
    ...
    def get_threats(
        self, query_hash: str, limit=10, response_format="application/json"
    ) -> dict:
