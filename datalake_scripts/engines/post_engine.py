...
class CommentsPost(PostEngine):
    ...
    def _post_comment(
        ...
        """
        payload = {"content": comment, "visibility": visibility}
...
class BulkSearch(PostEngine, HandleBulkTaskMixin):
    ...
    def _handle_bulk_search_task(self, task_uuid):
        ...
        return self.handle_bulk_task(
            task_uuid,
            retrieve_bulk_result_url,
            timeout=self.OCD_DTL_MAX_BULK_SEARCH_TIME,
        )

    def get_threats(
        self,
        query_hash: str = None,
        query_body: BaseEngine.Json = None,
        ...
        response = self.datalake_requests(
            self.url, "post", post_body=body, headers=self._post_headers()
        )
        if not response:
            logger.error(
...
class BulkLookupThreats(PostEngine):
    ...
    def bulk_lookup_threats(
        self, threats: BaseEngine.Json, additional_headers, hashkey_only: bool = True
    ) -> dict:
        body = threats
        body["hashkey_only"] = hashkey_only
        return self.datalake_requests(
            self.url, "post", {**self._post_headers(), **additional_headers}, body
        )
...
class AtomValuesExtractor(PostEngine):
    ...
    def atom_values_extract(
        self, untyped_atoms: List[str], treat_hashes_like="file"
    ) -> dict:
        payload = {
